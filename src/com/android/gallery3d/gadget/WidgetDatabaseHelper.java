...
public class WidgetDatabaseHelper extends SQLiteOpenHelper {
    ...
    //检测小部件中数据库图片Uri是否要更新
    public void checkWidgetSinglePhotoUri(int oldId, int newId) {
        SQLiteDatabase db = null;
        Cursor cursor = null;
        try {
            db = getWritableDatabase();
            cursor = db.query(TABLE_WIDGETS, new String[]{FIELD_APPWIDGET_ID, FIELD_IMAGE_URI},
                    FIELD_IMAGE_URI + " is not null",
                    null, null, null, null);
            while (cursor.moveToNext()) {
                int widget_id = cursor.getInt(0);
                String uri = cursor.getString(1);

                if (oldId != ContentUris.parseId(Uri.parse(uri))) {
                    continue;
                }
                //更新Widget数据库中图片Uri
                ContentValues values = new ContentValues();
                values.put(FIELD_IMAGE_URI, MediaStore.Images.Media.EXTERNAL_CONTENT_URI.buildUpon()
                        .appendPath(String.valueOf(newId)).build().toString());
                db.update(TABLE_WIDGETS, values, FIELD_APPWIDGET_ID + " = " + widget_id, null);

                //更新完需要通知小部件更新
                Intent intent = new Intent();
                intent.setAction(AppWidgetManager.ACTION_APPWIDGET_UPDATE);
                intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS, new int[]{widget_id});
                GalleryAppImpl.getApplication().sendBroadcast(intent);
            }
        } catch (Exception e) {
        } finally {
            Utils.closeSilently(cursor);
            Utils.closeSilently(db);
        }
    }
}
