...
public class WidgetUtils {
    ...
    public static Bitmap createWidgetBitmap(MediaItem image) {
        if (image.mIsDrmFile) {
            Log.d(TAG, "createWidgetBitmap: DRM image can't be displayed. file path: " + image.getFilePath());
            return null;
        }
        Bitmap bitmap = image.requestImage(MediaItem.TYPE_THUMBNAIL)
                .run(ThreadPool.JOB_CONTEXT_STUB);
        if (bitmap == null) {
            Log.w(TAG, "createWidgetBitmap bitmap = null, return null -> " + image.toString());
            return null;
        }
    ...
    public static Bitmap createWidgetBitmap(Bitmap bitmap, int rotation) {
        ...
        Bitmap target = Bitmap.createBitmap(
                sStackPhotoWidth, sStackPhotoHeight, isLowRam ? Config.ARGB_4444 : Config.ARGB_8888);
