...
public class WidgetService extends RemoteViewsService {
    ...
    private static class PhotoRVFactory implements
        ...
        private RemoteViews getNoThumbnailView(int position) {
            RemoteViews rv = new RemoteViews(mApp.getAndroidContext().getPackageName(), R.layout.appwidget_no_thumbnail_item);
            if (mSource != null) {
                Intent intent = new Intent()
                        .setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP)
                        .setData(mSource.getContentUri(position));
                rv.setOnClickFillInIntent(R.id.fail_to_load_image, intent);
            }
            return rv;
        }
        ...
        public RemoteViews getViewAt(int position) {
            ...
            if (mSource != null) {
                ...
                Log.w(TAG, "getViewAt position = " + position + ", mSource is null, not to get bitmap");
                return null;
            }
            /* @} */
            if (bitmap == null) {
                Log.d(TAG, "getViewAt position = " + position + ", bitmap = null, return LoadingView");
                return getNoThumbnailView(position);//getLoadingView();
            }
            ...
            if (mSource != null) {
                ...
                // when widget is destroyed, mSource can be null
                Log.w(TAG, "getViewAt position = " + position + ", mSource is null, not to get content uri");
                return null;
            }
            /* @} */
            return views;
