...
public class SecureSource extends MediaSource {
    ...
    private static final int SECURE_CAMERA_ALBUM = 2;

    static {
        mMatcher.add("/secure/all/*", SECURE_ALBUM);
        mMatcher.add("/secure/unlock", SECURE_UNLOCK);
        mMatcher.add("/secure/camera/album", SECURE_CAMERA_ALBUM);
    }
    ...
    public MediaObject createMediaObject(Path path) {
        switch (mMatcher.match(path)) {
            ...
            case SECURE_CAMERA_ALBUM:
                return new SecureCameraAlbum(path, mApplication);
            default:
