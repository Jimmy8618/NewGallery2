...
class BucketHelper {
    ...
    private static void updateBucketEntriesFromTable(JobContext jc,
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                // Log.i(TAG, "BucketEntry INDEX_DATA = " + cursor.getString(INDEX_DATA));
    ...
    private static BucketEntry[] loadBucketEntriesFromFilesTable(
        ...
        try {
                ...
                while (cursor.moveToNext()) {
                    if ((typeBits & (1 << cursor.getInt(INDEX_MEDIA_TYPE))) != 0) {
                        ...
                        BucketEntry entry = new BucketEntry(cursor.getInt(INDEX_BUCKET_ID),
                                cursor.getString(INDEX_BUCKET_NAME), cursor.getString(INDEX_DATA));
            ...
            Log.e(TAG, "loadBucketEntriesFromFilesTable error", e);
        }
        if (GalleryUtils.isSupportRecentlyDelete()) {
            buffer.add(new BucketEntry(TrashManager.getBucketId(), "Trash", ""));
        }
    ...
    private static String getBucketNameInTable(
        ...
        try {
            ...
            Log.w(TAG, "Gallery permissions is error,can't get data.");
            return null;
        } finally {
            Utils.closeSilently(cursor);
        }
    ...
    private static Uri getFilesContentUri() {
        return Files.getContentUri(EXTERNAL_MEDIA).buildUpon()
    ...
    public static class BucketEntry {
        ...
        public BucketEntry(int id, String name, String filename) {
            bucketId = id;
            bucketName = Utils.ensureNotNull(name);
            bucketFilename = Utils.ensureNotNull(filename);
            //Log.d(TAG, " BucketEntry bucketFilename =  " + bucketFilename + " bucketName = " + bucketName);
        }
        ...
        public boolean isMyAlbum() {
            return NewAlbumDialog.isMyAlbum(new File(bucketFilename).getParent());
        }
    }
}
