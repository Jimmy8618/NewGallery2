...
public class FilterDeleteSet extends MediaSet implements ContentListener {
    ...
    public void addSecureItems(long[] secureItemIds) {
        if (secureItemIds == null) {
            Log.d(TAG, "addSecureItems ids is null.");
            return;
        }
        if (mBaseSet instanceof SecureCameraAlbum) {
            ArrayList<Integer> ids = new ArrayList<>();
            for (long item : secureItemIds) {
                if (ids.contains((int) item)) {
                    continue;
                }
                ids.add((int) item);
            }
            Log.d(TAG, "addSecureItems size = " + ids.size());
            ((SecureCameraAlbum) mBaseSet).addSecureItems(ids);
        }
    }
    ...
    private void sendRequest(int type, Path path, int indexHint) {
        ...
        notifyContentChanged(Uri.parse("content://com.android.gallery3d.data.FilterDeleteSet/sendRequest"));
    }
