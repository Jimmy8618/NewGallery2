...
class LocalSource extends MediaSource {
        ...
        switch (mMatcher.match(path)) {
            ...
            case LOCAL_ALL_ALBUM: {
                ...
                return new LocalMergeAlbum(
                        path, new MediaSet[]{imageSet, videoSet}, bucketId);
            }
    ...
    public Path findPathByUri(Uri uri, String type) {
        Log.d(TAG, "findPathByUri: uri=" + uri + ", type=" + type + " match " + mUriMatcher.match(uri));
        try {
       ...
       /* SPRD : add to support play gif
        ...
        if (flag) {
            ...
            if ("com.android.camera.action.REVIEW".equalsIgnoreCase(action)) {
                return Path.fromString("/local/all").getChild(
                        String.valueOf(((LocalMediaItem) object).getBucketId()));
            } else if (object instanceof LocalImage) {
