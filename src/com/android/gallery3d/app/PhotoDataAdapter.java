...
public class PhotoDataAdapter implements PhotoPage.Model {
    ...
    private static class MySynchronizedHandler extends SynchronizedHandler {
        private final WeakReference<PhotoDataAdapter> mPhotoDataAdapter;

        public MySynchronizedHandler(GLRoot root, PhotoDataAdapter photoDataAdapter) {
            super(root);
        ...
        public void handleMessage(Message message) {
            PhotoDataAdapter photoDataAdapter = mPhotoDataAdapter.get();
    ...
    private MediaItem getItem(int index) {
        ...
        if (mActiveEnd == 0) {
            Log.i(TAG, "getItem fail! index = " + index + " mSize = " + mSize + "ActiveStart-End = " + mActiveStart + " - " + mActiveEnd);
            return null;
        }
    ...
    private void updateCurrentIndex(int index) {
        boolean isLoading = (mReloadTask != null && mReloadTask.mIsLoading.get());
        Log.d(TAG, "updateCurrentIndex " + mCurrentIndex + " --> " + index + ", isLoading = " + isLoading);
        if (mCurrentIndex == index || isLoading) {
            return;
        }
    ...
    public ScreenNail getScreenNail(int offset) {
        ...
        Log.i(TAG, "getScreenNail index = " + index + " mSize = " + mSize + "ActiveStart-End = " + mActiveStart + " - " + mActiveEnd);
        if (mActiveEnd == 0) {
            return null;
        }
        Utils.assertTrue(index >= mActiveStart && index < mActiveEnd);
    ...
    private void updateTileProvider(ImageEntry entry) {
        ...
        if (screenNail != null) {
            if (fullImage != null) {
                ...
                Log.d(TAG, " updateTileProvider fullImage: " + fullImage + ", " + w + " x " + h);
                mTileProvider.setScreenNail(screenNail, w, h);
    ...
    private class UpdateContent implements Callable<Void> {
        ...
        @Override
        public Void call() {
            GalleryUtils.start(this.getClass(), "call: UpdateContent call");
            ...
            fireDataChange();
            GalleryUtils.end(this.getClass(), "call: UpdateContent call");
            return null;
        }
    }
    ...
    private class ReloadTask extends Thread {
        ...
        private void updateLoading(boolean loading) {
            if (mIsLoading.compareAndSet(!loading, loading)) {
            ...
            while (mActive) {
                ...
                if (info.version != version) {
                    ...
                    if (info.size != 0 && mSize != info.size) {
                        Log.d(TAG, "ReloadTask, update size, contentStart, contentEnd");
                        mSize = info.size;
                        updateSlidingWindow();
                        Log.d(TAG, "mActiveStart = " + mActiveStart + ", mActiveEnd = " + mActiveEnd);
                        // after updateSlidingWindow, update contentStart, contentEnd
                        info.contentStart = mContentStart;
                        info.contentEnd = mContentEnd;
                        Log.d(TAG, "ReloadTask info size-start-end = " + info.size +
