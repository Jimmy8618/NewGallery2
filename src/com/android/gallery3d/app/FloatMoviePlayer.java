...
public class FloatMoviePlayer implements OnBufferingUpdateListener,
    ...
    private LinearLayout mLinear_show_button;
    ...
    private boolean mDrmRightConsumed = false;
    ...
    public FloatMoviePlayer(Context context) {
        ...
        mMediaPlayer = StandardFrameworks.getInstances().getMediaPlayerEx();
    ...
    private static class MyHandler extends Handler {
        private final WeakReference<FloatMoviePlayer> mFloatMoviePlayer;

        public MyHandler(FloatMoviePlayer floatMoviePlayer) {
            mFloatMoviePlayer = new WeakReference<FloatMoviePlayer>(floatMoviePlayer);
        }

        @Override
        public void handleMessage(Message msg) {
            FloatMoviePlayer floatMoviePlayer = mFloatMoviePlayer.get();
            if (floatMoviePlayer == null) {
                return;
            }
            floatMoviePlayer.updatePlayerPosition();
    ...
    public void alertVideoError(int messageId) {
        ...
        if (currentPlayBackMode == ALL_PLAYBACK) {
            alertDialog = new AlertDialog.Builder(mContext)
                    ...
                    .setPositiveButton(R.string.ok,//com.android.internal.R.string.VideoView_error_button,
    ...
    public void setPhoneState(boolean phoneState) {
        ...
        if (isPhoneCallActivityTop) {
            ...
            // Bug 934911,Continue to play the video when hangs up.
            start();
        }
    }
    ...
    public void start() {
        ...
        if (!mIsLayoutAdded || !mIsSurfaceCreated) {
            return;
        }
        /* @} */
        ...
        if (!StandardFrameworks.getInstances().getIsAudioFocusExclusive(mAm)) {
    ...
    public void setVideoUri(Uri videoUri) {
        try {
            if (null != mUri && null != mMediaPlayer && currentPlayBackMode == 0 && mCanSeekForward && mCanSeekBack) {
            ...
            if (mMediaPlayer != null) {
                ...
                if ((VideoDrmUtils.getInstance().isDrmFile() || SomePageUtils.getInstance().checkIsDrmFile(getFilePathFromUri(mUri))) && !mDrmRightConsumed) {
                    Log.d(TAG, "consumeDrmRights");
                    StandardFrameworks.getInstances().consumeDrmRights(mMediaPlayer);
                }
                mDrmRightConsumed = false;
                String uriString = mUri.toString();
                if (uriString.startsWith("https://") || uriString.startsWith("http://")
                        || uriString.startsWith("rtsp://")) {
                    // mSurfaceView.destroyDrawingCache();
    ...
    public void onPrepared(MediaPlayer player) {
        ...
        MediaPlayerMetaData data =
                StandardFrameworks.getInstances().getPlayerMetaData(player);
        mCanSeekBack = data.mCanSeekBack;
        mCanSeekForward = data.mCanSeekForward;
        mVideoDuration = mMediaPlayer.getDuration();//add by bug 780587.
    ...
    public void onCompletion(MediaPlayer arg0) {
        if (Settings.canDrawOverlays(mContext) == false) {
            onCompletion();
            closeWindow();
            return;
        }
        ...
        /*if (mPlayList != null) {
            if (currentPlayBackMode == SINGLE_PLAYBACK) {
                ...
                if (mCanSeekForward && mCanSeekBack) {
                    mVideoPosition = 0;
    ...
    public void addToWindow() {
        ...
        mWmParams.type = LayoutParams.TYPE_APPLICATION_OVERLAY; // set window type
        ...
        mSurfaceView = mFloatLayout.findViewById(R.id.float_surfaceview);
        ...
        mPausePlayBtn = mFloatLayout.findViewById(R.id.btn_pause);
        mPausePlayBtn.setOnClickListener(this);
        mBackToNormalBtn = mFloatLayout.findViewById(R.id.btn_back_to_normal);
        mBackToNormalBtn.setOnClickListener(this);
        mCloseWindowBtn = mFloatLayout.findViewById(R.id.btn_close_win);
        mCloseWindowBtn.setOnClickListener(this);
        mVideoLoadingTextView = mFloatLayout.findViewById(R.id.video_load);
        mLinear_show_button = mFloatLayout.findViewById(R.id.linear_show_button);
        /* SPRD: Modify for bug578015 System crashed at com.android.gallery3d @{ */
    ...
    public void showBtn() {
        ...
        mLinear_show_button.setVisibility(View.VISIBLE);
        String uriString = mUri.toString();
        if (uriString.startsWith("https://") || uriString.startsWith("http://")
                || uriString.startsWith("rtsp://")) {
        }
    ...
    public void hiddenBtn() {
        ...
        mLinear_show_button.setVisibility(View.INVISIBLE);
    }
    ...
    private void goToNormalPlay() {
        ...
        removeFromWindow();
        Intent intent;
        //add for Bug 915701 Go version has no NewVideoActivity UI
        boolean isGoVersion = StandardFrameworks.getInstances().getBooleanFromSystemProperties("ro.config.low_ram", false);
        if (mCurrentState == STATE_PLAYBACK_COMPLETED && !isGoVersion) {
        ...
        Log.d(TAG, "mCanSeekBack = " + mCanSeekBack + " mCanSeekForward = " + mCanSeekForward
                + " mUri = " + mUri + " state = " + state + " mDataIntent = " + mDataIntent
                + " isGoVersion = " + isGoVersion);
        /**SPRD:532462
    ...
    public void setDataIntent(Intent intent) {
        ...
        mDrmRightConsumed = mDataIntent.getBooleanExtra(KEY_DRM_VIDEO_CONSUMED, false);
    ...
    private void initPlayList(Boolean isStartByGallery) {
        ...
        switch (mFragmentName) {
            ...
            case HISTORY_VIDEOS_FRAGMENT:
                ...
                if (mInfo.mAlbum != null && !"".equals(mInfo.mAlbum)) {
                    Uri url = MediaStore.Video.Media.EXTERNAL_CONTENT_URI.buildUpon().
                            appendPath(String.valueOf(mInfo.mID)).build().parse(mInfo.mAlbum);
                    mInfo.mPath = url.getLastPathSegment();
                }
                playList2.add(mInfo);
                mPlayList = playList2;
                break;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_pause:
                if (!isPhoneCallActivityTop) {
                    if (mCurrentState == STATE_PLAYING) {
                        ...
                        if (mCurrentState == STATE_PLAYBACK_COMPLETED) {
                            VideoDrmUtils.getInstance().getFilePathByUri(mUri, mContext);
                            if (VideoDrmUtils.getInstance().isDrmFile() || SomePageUtils.getInstance().checkIsDrmFile(getFilePathFromUri(mUri))) {
                                VideoDrmUtils.getInstance().checkRightBeforeChangeInService(mContext, this);
                            } else {
                                start();
                            }
                        } else {
            ...
            case R.id.btn_close_win:
                mIsAbandonAudiofocus = true;
                if (null != mMediaPlayer && mCanSeekBack && mCanSeekForward) {
                    mVideoPosition = mMediaPlayer.getCurrentPosition();
                    mBookmarker.setBookmark(mBookMarkPath, mVideoPosition,
                            mVideoDuration);
                }
    ...
    private void changeVideo() {
        ...
        VideoDrmUtils.getInstance().getFilePathByUri(uri, mContext);
        if (VideoDrmUtils.getInstance().isDrmFile() || SomePageUtils.getInstance().checkIsDrmFile(getFilePathFromUri(uri))) {
            VideoDrmUtils.getInstance().checkRightBeforeChangeInService(mContext, this);
        } else {
            _changeVideo();
        }
    ...
    private static class MyAsyncQueryHandler extends AsyncQueryHandler {
        private WeakReference<FloatMoviePlayer> mFloatMoviePlayer;

        public MyAsyncQueryHandler(FloatMoviePlayer floatMoviePlayer, ContentResolver contentResolver) {
        ...
        protected void onQueryComplete(int token, Object cookie,
            ...
            FloatMoviePlayer floatMoviePlayer = mFloatMoviePlayer.get();
            if (floatMoviePlayer != null) {
                floatMoviePlayer.queryCompleteCallback(cursor);
            }
        }
    }

    private void queryCompleteCallback(Cursor cursor) {
        try {
    ...
    private void getVideoName(Intent intent) {
        ...
        if (mTitle != null) {
            ...
            AsyncQueryHandler queryHandler = new MyAsyncQueryHandler(this, mContext.getContentResolver());
    ...
    public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
        ...
        // UNISOC added for bug 1032183, Popup window when the problem video is played in the floating window.
        mCurrentState = STATE_ERROR;
        mTargetState = STATE_ERROR;
        /*SPRD:bug614698 When play all back ,the dialog is show all the time@{*/
        ...
        // Bug 946060, judge the error, Video can be still played normally.
        if (framework_err == FLUSH_ERROR && impl_err == 0 && !mMediaPlayer.isPlaying() && mCurrentState > STATE_PREPARED) {
            return true;
        }
    ...
    public boolean isCallingState() {
        if (mTelephonyManager == null
                || mSubscriptionManager == null || mContext == null) {
            return false;
        }
        ...
        if ("content".equalsIgnoreCase(scheme)) {
            if (mPlayList != null) {
                ...
                for (int i = 0; i < mPlayList.size(); i++) {
                    if (UriID.equalsIgnoreCase(mPlayList.get(i).mID)) {
                        try {
                            mBookMarkPath = Uri.parse(mPlayList.get(i).mAlbum);
                        } catch (NullPointerException e) {
                            mBookMarkPath = null;
