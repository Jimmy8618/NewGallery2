...
public class SaveImage {
    ...
    public static final int MAX_IMAGE_PIXEL_COUNT = 4000 * 4000;
    ...
    public static final String DEFAULT_SAVE_DIRECTORY = Environment.DIRECTORY_PICTURES + "/" + "EditedOnlinePhotos";
    public static final String MOTION_SNAP_SAVE_DIRECTORY = Environment.DIRECTORY_PICTURES + "/" + "MotionSnapPhotos";
    public static final String NEW_ALBUM_SAVE_DIRECTORY = Environment.DIRECTORY_DCIM + "/" + "Album";
    ...
    public boolean putExifData(File file, ExifInterface exif, Bitmap image,
        ...
        try {
            if (!GalleryStorageUtil.isInInternalStorage(file.getAbsolutePath())) {
                //创建文件
                SdCardPermission.mkFile(file);
                //外部存储需要用ParcelFileDescriptor访问文件
                pfd = SdCardPermission.createExternalFileDescriptor(file.getAbsolutePath(), "rw");
            }
            s = exif.getExifWriterStream(file.getAbsolutePath(), pfd);
            ...
            if (!ret) {
                if (!GalleryStorageUtil.isInInternalStorage(file.getAbsolutePath())) {
                    //外部存储删除文件
                    SdCardPermission.deleteFile(file.getAbsolutePath());
                } else {
                    file.delete();
                }
            }
    ...
    public Uri processAndSaveImage(ImagePreset preset, boolean flatten,
        ...
        InputStream is = null;
        try {
            is = mContext.getContentResolver().openInputStream(mSourceUri);
            BitmapFactory.Options options = new BitmapFactory.Options();
            options.inJustDecodeBounds = true;
            BitmapFactory.decodeStream(is, null, options);
            maxImageSize = Math.max(options.outWidth, options.outHeight);
            imagePixelCount = options.outWidth * options.outHeight;
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        } finally {
            Utils.closeSilently(is);
        ...
        if (isLowRam /*|| imagePixelCount > MAX_IMAGE_PIXEL_COUNT*/) {
            DisplayMetrics metrics = mContext.getResources().getDisplayMetrics();
            int maxDisplaySize = Math.max(metrics.widthPixels, metrics.heightPixels);

            if (maxImageSize != 0 && maxImageSize > maxDisplaySize) {
                sampleSize = maxImageSize / maxDisplaySize;
            }
        }
            ...
            if (flatten) {
                ...
                if (putExifData(mDestinationFile, exif, mPreviewImage, quality)) {
                    ...
                    if (Build.VERSION.SDK_INT >= ApiHelper.VERSION_CODES.Q) {
                        insertUri = Utils.getImageMediaDataBaseUri(mContext, mDestinationFile.getAbsolutePath());
                    } else {
                        insertUri = Images.Media.EXTERNAL_CONTENT_URI;
                    }
            ...
            try {
                ...
                if (putExifData(mDestinationFile, exif, bitmap, quality)) {
                    ...
                    if (!flatten) {
                        ...
                        Uri updateUri;
                        if (Build.VERSION.SDK_INT >= ApiHelper.VERSION_CODES.Q) {
                            updateUri = Utils.getImageMediaDataBaseUri(mContext, mDestinationFile.getAbsolutePath());
                        } else {
                            updateUri = Images.Media.EXTERNAL_CONTENT_URI;
                        }
                        mContext.getContentResolver().update(updateUri,
    ...
    public static Uri makeUri(Context context, Uri sourceUri) {
        long time = System.currentTimeMillis();
        String filename = GalleryUtils.generateImageName(time);
        File sourceDir = getSaveDirectory(context, sourceUri);
        String saveDirectory;
        String relativePath;
        if (sourceDir == null) {
            saveDirectory = Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + DEFAULT_SAVE_DIRECTORY;
        } else {
            String storageDir = Utils.getStorageDirectory(sourceDir.getAbsolutePath());
            if (storageDir == null) {
                saveDirectory = Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + DEFAULT_SAVE_DIRECTORY;
            } else {
                saveDirectory = storageDir + "/" + DEFAULT_SAVE_DIRECTORY;
            }
        }
        relativePath = DEFAULT_SAVE_DIRECTORY;
        filename = Utils.generateFileName(saveDirectory, filename, POSTFIX_JPG);
        return linkUri(context, new File(saveDirectory + "/" + filename + POSTFIX_JPG), relativePath, time);
    }

    private static Uri linkUri(Context context, File file, String relativePath, long time) {
        final ContentValues values = new ContentValues();

        time /= 1000;
        values.put(Images.Media.RELATIVE_PATH, relativePath);
        ...
        Uri c = Images.Media.getContentUri(Utils.getMediaVolumeName(context, file.getAbsolutePath()));

        return context.getContentResolver().insert(c, values);
    }
    ...
    public static Uri linkNewFileToUri(Context context, Uri sourceUri,
        ...
        if (fileUri || oldSelectedFile == null || !deleteOriginal) {
            Uri insertUri;
            if (Build.VERSION.SDK_INT >= ApiHelper.VERSION_CODES.Q) {
                insertUri = Utils.getImageMediaDataBaseUri(context, file.getAbsolutePath());
            } else {
                insertUri = Images.Media.EXTERNAL_CONTENT_URI;
            }
    ...
    public static Uri updateFilePending(Context context, Uri uri, int width, int height) {
        if (null == uri) {
            Log.d(LOGTAG, "updateFilePending uri is null");
            return null;
        }
        int fileSize = 0;
        InputStream is = null;
    ...
    private void updateExifData(ExifInterface exif, long time, short w, short h) {
        ...
        exif.setTag(exif.buildTag(ExifInterface.TAG_CAMERATYPE_IFD, LocalImage.IMG_TYPE_MODE_NORMAL));
    }
    /* @} */
}
