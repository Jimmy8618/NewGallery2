...
public class ImageStraighten extends ImageShow {
    ...
    public static int mSavedAngle = 0;
    ...
    */
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mFirstDrawSinceUp) {
            ...
            int count = getActivity().getHistoryCount();
            android.util.Log.d(TAG, " ImageStraighten onDraw count =  " + count);
            if (mAngle != 0) {
                getActivity().saveMenuItem(true);
            } else if (count == 2 && mAngle == 0) {
                getActivity().saveMenuItem(false);
            }
        }
    ...
    @Override
    public void onSeekBarProgressChanged(int progress, int angle) {
        // TODO Auto-generated method stub
        super.onSeekBarProgressChanged(progress, angle);
        if (mState == MODES.MOVE) {
            mAngle = angle;
            invalidate();
            mSavedAngle = angle;
