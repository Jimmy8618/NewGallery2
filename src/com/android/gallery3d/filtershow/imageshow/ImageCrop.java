...
import com.android.gallery3d.filtershow.filters.FilterCropRepresentation.CropRatio;
...
public class ImageCrop extends ImageShow {
    ...
    private void setAspectRatio() {
        ...
        CropRatio ratio = mEditorCrop.getCropRatio();
        if (ratio == CropRatio.Ratio_Free) {
            mCropObj.unsetAspectRatio();
        } else if (ratio == CropRatio.Ratio_4_3) {
            setAspect(4, 3);
        } else if (ratio == CropRatio.Ratio_16_9) {
            setAspect(16, 9);
        } else if (ratio == CropRatio.Ratio_Square) {
            setAspect(1, 1);
        } else if (ratio == CropRatio.Ratio_Original) {
            setOriginalAspect();
        }
    }
    ...
    private void setAspect(float x, float y) {
        if (x <= 0 || y <= 0) {
            throw new IllegalArgumentException("Bad arguments to applyAspect");
        }
        // If we are rotated by 90 degrees from horizontal, swap x and y
