...
public class BoundedRect {
    private static final String TAG = "BoundedRect";
    private float rot;
    private RectF outer;
    private RectF inner;
    ...
    public void moveInner(float dx, float dy) {
        Log.d(TAG, "moveInner: dx=" + dx + ", dy=" + dy);
        ...
        Log.d(TAG, "moveInner: outerCorners = " + outerCorners);
        ...
        for (int i = 0; i < translatedInnerCorners.length; i += 2) {
            ...
            if (!CropMath.inclusiveContains(outer, correctedInnerX, correctedInnerY)) {
                ...
                Log.d(TAG, "moveInner: nearestSide = " + nearestSide);
                // UNISOC added for bug 1207869, avoid gallery crash.
                if (nearestSide == null) {
                    return;
                }
                float[] correctionVec =
