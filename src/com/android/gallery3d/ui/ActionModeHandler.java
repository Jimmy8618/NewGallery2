...
public class ActionModeHandler implements Callback, PopupList.OnPopupItemClickListener {
    ...
    private boolean mEditAlbums = false;
    ...
    public ActionModeHandler(
        ...
        mMenuExecutor = new MenuExecutor(activity, activity.getGLRoot(), selectionManager);
    ...
    public void startActionMode() {
        ...
        mSelectionMenu = new SelectionMenu(a,
                (TextView) customView.findViewById(R.id.selected_count), this);
        updateSelectionMenu();
    }
    ...
    public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
        if (!mIsActive) {
            Log.d(TAG, "onActionItemClicked action mode is NOT active!");
            return false;
        }
    ...
    public void updateSelectionMenu() {
        ...
        // update title
        int count = 0;
        if (mSelectionManager.inAlbumSelectionMode()) {
            count = mSelectionManager.getSelectedAlbumCount();
        } else {
...
//        setTitle(String.format(format, count));
    ...
    public boolean onCreateActionMode(ActionMode mode, Menu menu) {
        ...
        if (mEditAlbums) {
            mode.getMenuInflater().inflate(R.menu.save_menu, menu);
        } else {
        ...
        mActivity.setStatusBarColor(mActivity.getColor(R.color.selection_statusbar_color));
        mIsActive = true;
        return true;
    }
    ...
    public void onDestroyActionMode(ActionMode mode) {
        ...
        mMainHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                mActivity.setStatusBarTranslucent();
            }
        }, 500);
    }
    ...
    private int computeMenuOptions(ArrayList<MediaObject> selected) {
        ...
        switch (selected.size()) {
            case 1:
                ...
                if (!GalleryUtils.isCropAvailable(mActivity, mimeType)) {
    ...
    private void share() {
        ...
        ArrayList<Path> expandedPaths = mSelectionManager.getSelected(true, MAX_SELECTED_ITEMS_FOR_SHARE_INTENT);
        if (expandedPaths == null || expandedPaths.size() == 0) {
            return;
        }
        if (expandedPaths.size() == 1) {
            DataManager manager = mActivity.getDataManager();
            MediaObject mediaObject = manager.getMediaObject(expandedPaths.get(0));
            if (mediaObject != null && mediaObject instanceof MediaItem) {
                MenuExecutor.launchShareIntent(mActivity, (MediaItem) mediaObject);
            }
        } else {
            Intent intent = computeSharingIntent(null, MAX_SELECTED_ITEMS_FOR_SHARE_INTENT);
    ...
    public void updateSupportedOperation() {
        ...
        Log.d(TAG, "updateSupportedOperation submitInThread");
        mMenuTask = mActivity.getThreadPool().submitInThread(new Job<Void>() {
            @Override
            public Void run(final JobContext jc) {
                Log.d(TAG, "updateSupportedOperation MenuTask run.");
                ...
                if (selected == null) {
                    mMainHandler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            MenuExecutor.updateMenuOperation(mMenu, 0, false);
                        }
                    });
                    return null;
                }
                ...
                final boolean canShare = numSelected >= 1;
                ...
                mMainHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        MenuExecutor.updateMenuOperation(mMenu, operation, false);
    ...
    public void setEditAlbumFlag(boolean isEdit) {
        Log.i(TAG, "setEditAlbumFlag  isEdit = " + isEdit);
        mEditAlbums = isEdit;
    }
