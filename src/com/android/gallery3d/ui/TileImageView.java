...
public class TileImageView extends GLView {
    ...
    public TileImageView(Activity context, PhotoView photoView) {
        mPhotoView = photoView;
        mThreadPool = GalleryAppImpl.getApplication().getThreadPool();
    ...
    public void prepareTextures() {
        ...
        if (mIsTextureFreed) {
            ...
            if (mModel != null) {
                ...
                // if mTexture of screenNail is null, It do't need to setScreenNail
                if (!(screenNail instanceof TiledScreenNail) ||
                        ((TiledScreenNail) screenNail).getTexture() != null) {
    ...
    protected void render(GLCanvas canvas) {
        ...
        try {
            if (level != mLevelCount && !isScreenNailAnimating()) {
                ...
                // Modify for Bug 1033745, playing video in GLSurfaceView
                // 如果没有播放视频, 则渲染图片
                if (getGLRoot() == null || !getGLRoot().isGLVideoPlaying()) {
            ...
            // 回调渲染视频接口
            if (getGLRoot() != null) {
                getGLRoot().renderGLVideo(canvas, mOffsetX, mOffsetY,
                        Math.round(mImageWidth * mScale),
                        Math.round(mImageHeight * mScale));
            }
        } finally {
    ...
    private synchronized void uploadBackgroundTiles(GLCanvas canvas) {
    ...
    synchronized void queueForDecode(Tile tile) {
        if (tile.mTileState == STATE_ACTIVATED) {
            ...
            if (mDecodeQueue.push(tile) && mPhotoView.getScaleState()) {
                notifyAll();
            }
        }
    }
            ...
            while (quota > 0) {
                synchronized (TileImageView.this) {
                    ...
                    if (!tile.isContentValid()) {
                        ...
                        //Log.d(TAG, "TileUploader.onGLIdle tile.mTileState=" + tile.mTileState);
    ...
    public void drawTile(GLCanvas canvas,
        ...
        if (tile != null) {
            if (!tile.isContentValid()) {
                ...
                } else if (tile.mTileState == STATE_IN_QUEUE) {
                    //do nothing
    ...
    private class Tile extends UploadedTexture {
        ...
        protected Bitmap onGetBitmap() {
            //Log.d(TAG, "Tile.onGetBitmap mTileState=" + mTileState);
