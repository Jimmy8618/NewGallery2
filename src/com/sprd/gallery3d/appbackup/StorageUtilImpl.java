...
class StorageUtilImpl extends StorageUtil {
    ...
    static {
        MMC_SUPPORT = StandardFrameworks.getInstances().
                getBooleanFromSystemProperties("ro.device.support.mmc", false);
        // SPRD: EXTERNAL_STORAGE_DIRECTORY is not return right, so restore failed. @{
        String path = null;
        File externalFile = null;
        externalFile = StandardFrameworks.getInstances().getExternalStoragePath(GalleryAppImpl.getApplication());
        ...
        internalFile = StandardFrameworks.getInstances().getInternalStoragePath();
    ...
    private static String getMainStoragePathKey() {
        ...
        try {
            ...
            switch (StandardFrameworks.getInstances().getIntFromSystemProperties("persist.storage.type", -1)) {
                case 0:
                    mIsNAND = true;
                    return "EXTERNAL_STORAGE";
                case 1:
            ...
            if (EXTERNAL_STORAGE_DIRECTORY.canRead()) {
                ...
                Log.d(TAG,
                        "backup Double Check storage is canread ="
                                + String.valueOf(StandardFrameworks.getInstances().
                                getExternalStoragePath(GalleryAppImpl.getApplication()).canRead()));
                return true;
            }
        } catch (Exception rex) {
            return false;
