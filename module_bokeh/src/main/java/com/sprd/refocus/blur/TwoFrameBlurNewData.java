...
public class TwoFrameBlurNewData extends RefocusData {
    ...
    private int version;
    private int dataVersion; //hal数据结构版本号
    private String blurFlag;
    //private int oriJpegSize; //for new blur3.0 data(cal weightMap in camera)
    private int farJpegSize; //for new blur3.0 data(cal weightMap in galley)
    private int nearJpegSize; //for new blur3.0 data(cal weightMap in galley)

    public TwoFrameBlurNewData(byte[] content) {
    ...
    public void initDatas(byte[] content) {
        ...
        version = RefocusUtils.getIntValue(content, 3);
        ispTunning = RefocusUtils.getIntValue(content, 4); // 4 isp tunning? 0 no, 1 yes
        calWeightMap = RefocusUtils.getIntValue(content, 5); //5  0: don't need to calculate depth, 1: need calculate
        sel_y = RefocusUtils.getIntValue(content, 6); //6 mSelCoordY
        sel_x = RefocusUtils.getIntValue(content, 7); // 7 mSelCoordX
        blurIntensity = RefocusUtils.getIntValue(content, 8);   // 8mFNum
        yuvHeight = RefocusUtils.getIntValue(content, 9);  // 9 mMainHeightData
        yuvWidth = RefocusUtils.getIntValue(content, 10); // 10 mMainWidethData
        farJpegSize = RefocusUtils.getIntValue(content, 11); // 11 farJpeg size
        nearJpegSize = RefocusUtils.getIntValue(content, 12); // 12 nearJpeg size
        rotation = RefocusUtils.getIntValue(content, 13); // 13 rotation

        tmp_thr = RefocusUtils.getIntValue(content, 14); // 14 tmp_thr
        tmp_mode = RefocusUtils.getIntValue(content, 15); // 15 tmp_mode
        similar_factor = RefocusUtils.getIntValue(content, 16); // 16 similar_factor
        merge_factor = RefocusUtils.getIntValue(content, 17); // 17 merge_factor
        refer_len = RefocusUtils.getIntValue(content, 18); // 18 refer_len
        scale_factor = RefocusUtils.getIntValue(content, 19); // 19 scale_factor
        touch_factor = RefocusUtils.getIntValue(content, 20); // 20 touch_factor
        smooth_thr = RefocusUtils.getIntValue(content, 21); // 21 smooth_thr
        depth_mode = RefocusUtils.getIntValue(content, 22); // 22 depth_mode
        fir_edge_factor = RefocusUtils.getIntValue(content, 23); // 23 fir_edge_factor
        fir_cal_mode = RefocusUtils.getIntValue(content, 24); // 24 fir_cal_mode
        fir_channel = RefocusUtils.getIntValue(content, 25); // 25 fir_channel
        fir_len = RefocusUtils.getIntValue(content, 26); // 26 fir_len
        fir_mode = RefocusUtils.getIntValue(content, 27); // 27 fir_mode
        enable = RefocusUtils.getIntValue(content, 28); // 28 enable
        ...
        //29-44
        for (int i = 0; i < 7; i++) {
            hfir_coeff[i] = RefocusUtils.getIntValue(content, 35 - i);
        }

        //36-42
        for (int i = 0; i < 7; i++) {
            vfir_coeff[i] = RefocusUtils.getIntValue(content, 42 - i);
        }

        //43 -45
        for (int i = 0; i < 3; i++) {
            similar_coeff[i] = RefocusUtils.getIntValue(content, 45 - i);
        }

        //46-53
        for (int i = 0; i < 8; i++) {
            tmp_coeff[i] = RefocusUtils.getIntValue(content, 53 - i);
        }

        mainYuvSize = yuvHeight * yuvWidth * 3 / 2;  //9 main_yuv_data  size is w * h* 3/2
        oriJpegSize = nearJpegSize;
        if (calWeightMap == 0) {
            int weightMapSize = yuvHeight * yuvWidth / 4;    //10 weight_map size is w * h / 4
            jpegSize = content.length - nearJpegSize - weightMapSize - 53 * 4;
            ...
            jpegSize = content.length - nearJpegSize - farJpegSize - 53 * 4;
            farJpeg = new byte[farJpegSize];
            nearJpeg = new byte[nearJpegSize];
            int farYuvIndex = jpegSize;
            int mainYuvIndex = farYuvIndex + farJpegSize;
            System.arraycopy(content, farYuvIndex, farJpeg, 0, farJpegSize);
            System.arraycopy(content, mainYuvIndex, nearJpeg, 0, nearJpegSize);
        }
    }
    ...
    public String toString() {
        return "TwoFrameBlurData{" +
                ...
                ", \ntmp_thr=" + tmp_thr +
                ", \nrotation=" + rotation +
                ", \nnearJpegSize=" + nearJpegSize +
                ", \nfarJpegSize=" + farJpegSize +
                ", \nyuvWidth=" + yuvWidth +
                ", \nyuvHeight=" + yuvHeight +
