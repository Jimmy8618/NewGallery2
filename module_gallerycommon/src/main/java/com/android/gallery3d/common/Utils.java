...
public class Utils {
     ...
     * @param filePath 文件路径
     * @return Media Volume名称
     */
    public static String getMediaVolumeName(Context context, String filePath) {
        try {
            String storageName = getStorageDirectory(filePath).toLowerCase();
            Set<String> volumeNames = MediaStore.getExternalVolumeNames(context);
            for (String vol : volumeNames) {
                if (storageName.contains(vol.toLowerCase())) {
                    return vol;
                }
            }
        } catch (Exception e) {
     ...
     * @return 新的文件名
     */
    public static String generateFileName(String dir, String fileName, String postfix) {
        String newName = fileName;
        int copy = 1;
        while (new File(dir + "/" + newName + postfix).exists()) {
    ...
    /**
     * 根据文件路径, 获取图片数据库的Uri, 内部存储和SD卡对应的Uri已不一致
     *
     * @param context  context
     * @param filePath 文件路径
     * @return 数据库Uri
     */
    public static Uri getImageMediaDataBaseUri(Context context, String filePath) {
        String volume = getMediaVolumeName(context, filePath);
        return MediaStore.Images.Media.getContentUri(volume);
    }

    /**
     * 根据文件路径, 获取视频数据库的Uri, 内部存储和SD卡对应的Uri已不一致
     *
     * @param context  context
     * @param filePath 文件路径
     * @return 数据库Uri
     */
    public static Uri getVideoMediaDataBaseUri(Context context, String filePath) {
        String volume = getMediaVolumeName(context, filePath);
